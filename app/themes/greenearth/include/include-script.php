	...
	*/ 
	...
	function register_non_admin_styles(){
		...
		// Page post_type
			...
			// If using custom slider
			if( $gdl_top_slider_type == 'Custom Slider' ){
				
				wp_enqueue_style('gdl-custom-slider',GOODLAYERS_PATH.'/stylesheet/gdl-custom-slider.css');
	...
	function register_non_admin_scripts(){
		...
		// Page post_type
			...
			// If using custom slider
			if( $gdl_top_slider_type == 'Custom Slider' ){
				$custom_setting = get_gdl_slider_option_array($goodlayers_element['gdl_panel_custom_slider']);		
		
				wp_deregister_script('custom-slider');
				wp_register_script('custom-slider', GOODLAYERS_PATH.'/javascript/jquery.custom-slider.js', false, '1.0', true);
				wp_localize_script( 'custom-slider', 'CUSTOM', $custom_setting);
